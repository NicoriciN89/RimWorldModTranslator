...
- **`src/translator.py`** — перевод через Argos Translate. Числовые
  плейсхолдеры (`{0}`, `{1}`) и литеральные `\n`/`\r` никогда не отдаются
  модели как текст — строка режется на сегменты по их границам, переводится
  только текст между ними, плейсхолдеры возвращаются на место буквально.
